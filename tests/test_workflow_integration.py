...
class TestWorkflowJobExecution:
    ...
    def test_pytest_collects_all_tests(self, project_root):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "--collect-only", "-q", 
             "tests/test_workflow.py", "tests/test_workflow_integration.py"],
            capture_output=True,
            text=True,
            cwd=project_root
        )
        ...
        assert result.returncode == 0, f"Test collection failed: {result.stdout}\n{result.stderr}"
        
        # Should find multiple test files
        output = result.stdout + result.stderr
        assert "test session starts" in output or "tests collected" in output or ".py" in output
    ...
    def test_python_module_imports_work(self, project_root):
        ...
        if result.returncode != 0:
            ...
            assert "module named" in result.stderr.lower() or result.returncode in [0, 1], \
...
class TestLintingTools:
    ...
    def test_flake8_can_run(self, project_root):
        ...
        assert result.returncode == 0 or "command not found" in result.stderr.lower() or \
               "no module named" in result.stderr.lower(), \
    ...
    def test_python_files_exist(self, project_root):
        ...
        expected_files = ['main.py', 'auth.py', 'crypto_utils.py', 'data_store.py', 'gui_login.py']
        for expected in expected_files:
            assert any(f.endswith(expected) for f in python_files), \
